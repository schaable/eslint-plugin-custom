...
module.exports = {
  meta: {
    ...
    messages: {
      ENFORCE_DYNAMIC_IMPORT: "This import transitively imports the slow dependency '{{dependency}}' in file '{{filename}}' at line {{line}}",
    },
  },
  ...
  create(context) {
    ...
    function visitor(filename, originalNode, node) {
      ...
      function detectTopLevelExternalDependency(path) {
        ...
        visit(ast, visitorKeys, visitModules(node => visitor(path, originalNode, node)));
      }
