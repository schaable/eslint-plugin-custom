...
function visitModules(visitor) {
  ...
  function visitImportDeclaration(node) {
    ...
    const hasTypeSpecifiers = node.specifiers > 0 && node.specifiers.every(({ importKind }) => importKind === 'type');
  ...
  const visitors = {
    ...
    'Program > VariableDeclaration > VariableDeclarator > CallExpression[callee.name="require"]': visitCallExpression,
